...
// Хешер для работы с std::unordered_map
class PositionHasher {
public:
	std::size_t operator()(const Position& pos) const noexcept {
		return _hasher(pos.row) + _hasher(pos.col * 37);
	}
private:
...
public:
	...
	FormulaError(Category category);                             // конструктор от категории ошибки
	Category GetCategory() const;                                // вернуть категорию ошибки
	bool operator==(FormulaError rhs) const;                     // проверить тип категории
	std::string_view ToString() const;                           // вернуть строковое представление ошибки
...
std::unique_ptr<SheetInterface> CreateSheet();       // реализация в конце sheet.cpp