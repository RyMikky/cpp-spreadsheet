...
namespace {
    ...
    void TestClearCell() {
        ...
        auto n_size = sheet->GetPrintableSize();
        ASSERT(n_size.cols == 0);
        ASSERT(n_size.rows == 0);
    }
...
int main() {
    ...
    RUN_TEST(tr, TestCacheCorrect); // доп тест

    // Дополнительные персональные ассерты из unit_test_system
